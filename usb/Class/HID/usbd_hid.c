...
static uint16_t hid_getDesc(USBD_HID_IfHandleType *itf, uint8_t ifNum, uint8_t * dest)
{
    ...
    if (HID_APP(itf)->Report->DevType == 1) {
      /* Keyboard */
      desc->HID.bInterfaceSubClass = 1;
      desc->HID.bInterfaceProtocol = 1;
    } else if (HID_APP(itf)->Report->DevType == 2) {
      /* Mouse */
      desc->HID.bInterfaceProtocol = 2;
    }
